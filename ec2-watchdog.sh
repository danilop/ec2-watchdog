...
# One of the main differences is that here you can check that a service is working
# on the other node with different protocols, i.e. using ICMP, TCP and/or HTTP/HTTPS.
...
# Other node's IP, TCP Port, HTTP/HTTPS URL to check, and EIP Allocation ID to swap if other node goes down
